...
;; kototama 用に com-junkawasaki/root superproject の manifest/edn-datomize.bb
;; （Phase 2 pilot: gftdcojp/net-kotobase 経由）から移植（Phase 3 fanout）。
;; schema-path をこの repo のルート schema.edn に向けている以外はロジック同一
;; ＋ wrap-generic モードを追加（既に名前空間付きキーを持つ map 用）。
;;
...
;; kototama (child repo) has no manifest/ dir — schema lives at repo root.
